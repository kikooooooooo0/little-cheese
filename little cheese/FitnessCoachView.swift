...
struct FitnessCoachView: View {
    ...
    private func generateRoutine() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            ...
            if totalMinutes > 20 && wPool.count > 1 { warmupActions.append(wPool[1]) }
            ...
            if cardioMinutes > 0 {
                ...
                let cardioAction = FitnessAction(
                    name: "去 \(cardioName) 挥洒 \(Int(cardioMinutes)) 分钟！",
            ...
            if totalMinutes > 30 && cPool.count > 1 { cooldownActions.append(cPool[1]) }
    ...
    private func getSmartWarmupPool(part: Int) -> [FitnessAction] {
        if part == 2 { // 练腿：强制加入髋部/臀部唤醒
            return [
                FitnessAction(name: "动态臀桥唤醒 (15次)", targetMuscle: "臀大肌", tip: "不要用腰顶，屁股夹紧发力", emojiIcon: "🌉", steps: ["仰卧屈膝，脚跟踩地", "快速顶起臀部，慢速放下"]),
                FitnessAction(name: "世界最伟大拉伸 (每侧5次)", targetMuscle: "髋关节活动度", tip: "动作一定要慢，感受髋部被打开", emojiIcon: "🌍", steps: ["弓步迈出，同侧手肘尽量触地", "随后同侧手臂向天花板展开扭转胸椎"])
            ].shuffled()
        } else { // 练背或核心：肩部与脊柱唤醒
            return [
                FitnessAction(name: "猫牛式脊柱活动 (10次)", targetMuscle: "脊柱与核心", tip: "动作缓慢，配合呼吸", emojiIcon: "🐈", steps: ["四足跪姿，吸气塌腰，呼气拱背"]),
                FitnessAction(name: "肩部环绕大风车 (每侧10圈)", targetMuscle: "肩关节", tip: "幅度尽量大", emojiIcon: "🚁", steps: ["双手伸直画大圈，前后各10圈"])
            ].shuffled()
        }
    }
    
    private func getSmartCooldownPool(part: Int) -> [FitnessAction] {
        if part == 2 { // 练腿冷身：靶向拉腿臀
            return [
                FitnessAction(name: "鸽子式臀部拉伸 (每侧45秒)", targetMuscle: "臀大肌", tip: "如果膝盖痛就立刻停止", emojiIcon: "🕊️", steps: ["前腿屈膝横放，后腿伸直", "上半身慢慢趴下感受臀部拉扯"]),
                FitnessAction(name: "大腿前侧站立拉伸 (每侧30秒)", targetMuscle: "股四头肌", tip: "保持身体直立，不要塌腰", emojiIcon: "🦩", steps: ["单手抓住同侧脚踝，将脚跟拉向臀部"])
            ].shuffled()
        } else if part == 1 { // 练背冷身：拉背 + 补偿拉胸
            return [
                FitnessAction(name: "婴儿式背部放松 (1分钟)", targetMuscle: "下背阔肌", tip: "尽情感受呼吸，放松全身", emojiIcon: "👶", steps: ["双膝跪地，臀部坐在脚跟，上半身趴下"]),
                FitnessAction(name: "胸部墙壁拉伸 (每侧30秒)", targetMuscle: "胸大肌补偿", tip: "防止练背导致圆肩", emojiIcon: "🚪", steps: ["单手小臂贴住墙面，身体向反方向扭转"])
            ].shuffled()
        } else {
            return [
                FitnessAction(name: "腹部眼镜蛇拉伸 (30秒)", targetMuscle: "腹直肌", tip: "骨盆贴地，肩膀下沉", emojiIcon: "🐍", steps: ["趴在垫子上，双手将上半身撑起"]),
                FitnessAction(name: "全身大字形放松 (1分钟)", targetMuscle: "全身神经", tip: "闭上眼睛，什么都不想", emojiIcon: "🧘‍♂️", steps: ["平躺在垫子上，手脚自然分开"])
            ].shuffled()
        }
    }
    ...
    private func getSmartBalancedPool(equip: Int, part: Int) -> [FitnessAction] {
        ...
        if part == 2 || part == 0 { // 🎯 下肢强制平衡逻辑：推 + 拉 + 臀
            if equip == 2 { // 健身房
                pool.append(FitnessAction(name: "倒蹬机 (Leg Press)", targetMuscle: "大腿前侧 (推)", tip: "顶端绝对不要锁死膝盖", emojiIcon: "🎢", steps: ["踩实踏板，慢下快推"]))
                pool.append(FitnessAction(name: "罗马椅挺身 / 腿弯举", targetMuscle: "大腿后侧 (拉)", tip: "感受大腿后侧拉扯力", emojiIcon: "🪑", steps: ["控制下放速度，臀腿发力拉起"]))
                pool.append(FitnessAction(name: "负重臀桥 / 髋推", targetMuscle: "臀大肌 (臀)", tip: "顶峰收缩夹紧屁股", emojiIcon: "🍑", steps: ["用杠铃或哑铃压在髋部，发力向上顶"]))
                pool.append(FitnessAction(name: "史密斯深蹲", targetMuscle: "臀腿综合", tip: "核心收紧，脚跟发力", emojiIcon: "🏋️", steps: ["背部挺直，下蹲至大腿平行地面"]))
            } else { // 哑铃/徒手
                pool.append(FitnessAction(name: "高脚杯深蹲", targetMuscle: "大腿前侧 (推)", tip: "保持挺胸，哑铃贴紧胸口", emojiIcon: "🍷", steps: ["手肘处于双膝之间下蹲"]))
                pool.append(FitnessAction(name: "哑铃罗马尼亚硬拉 (RDL)", targetMuscle: "大腿后侧 (拉)", tip: "背部绝对平直，臀部向后推", emojiIcon: "🚪", steps: ["哑铃贴着腿部滑下，感受后侧拉伸"]))
                pool.append(FitnessAction(name: "哑铃负重臀桥", targetMuscle: "臀大肌 (臀)", tip: "把哑铃放在小腹上方", emojiIcon: "🍑", steps: ["脚跟踩地，臀部发力向上顶"]))
                pool.append(FitnessAction(name: "交替箭步蹲", targetMuscle: "单侧臀腿", tip: "下蹲时前后腿呈90度", emojiIcon: "🚶‍♀️", steps: ["保持上身直立，重心在两腿中间"]))
            }
        } else if part == 1 { // 🎯 背部强制平衡逻辑：垂直拉 + 水平拉
            if equip == 2 {
                pool.append(FitnessAction(name: "器械高位下拉", targetMuscle: "背阔肌 (垂直拉)", tip: "不要过度后仰，拉向锁骨", emojiIcon: "🏗️", steps: ["大腿卡紧，背部发力带动手臂"]))
                pool.append(FitnessAction(name: "坐姿划船", targetMuscle: "中背部 (水平拉)", tip: "肩胛骨向后收缩夹紧", emojiIcon: "🚣", steps: ["挺胸收腹，手肘贴着肋骨向后拉"]))
                pool.append(FitnessAction(name: "器械反向飞鸟", targetMuscle: "肩后束", tip: "改善圆肩驼背神器", emojiIcon: "🦋", steps: ["手臂微屈，向后发力打开"]))
            } else {
                pool.append(FitnessAction(name: "弹力带高位下拉", targetMuscle: "背阔肌 (垂直拉)", tip: "找个高点固定弹力带", emojiIcon: "⚡", steps: ["跪姿或站姿，将弹力带拉向胸口"]))
                pool.append(FitnessAction(name: "哑铃俯身划船", targetMuscle: "中背部 (水平拉)", tip: "背部必须平直", emojiIcon: "🚣", steps: ["臀部后推，上身前倾，哑铃拉向小腹"]))
                pool.append(FitnessAction(name: "弹力带面拉", targetMuscle: "肩后束", tip: "拉向脸部，手臂呈W型", emojiIcon: "😎", steps: ["高度与头齐平，向后挤压肩胛骨"]))
            }
        } else { // 核心
            pool.append(FitnessAction(name: "平板支撑", targetMuscle: "整体核心", tip: "切忌塌腰", emojiIcon: "🪵", steps: ["收紧肚子，身体绷直像木板"]))
            pool.append(FitnessAction(name: "死虫子 (Deadbug)", targetMuscle: "深层核心", tip: "下背部必须死死贴住地面", emojiIcon: "🪲", steps: ["对侧手脚伸展，不碰地面"]))
            pool.append(FitnessAction(name: "负重俄罗斯挺身", targetMuscle: "腹外斜肌", tip: "转动胸椎而不是手臂", emojiIcon: "🌪️", steps: ["身体后倾45度，左右扭转"]))
        }
        
        return pool // 按照编排好的科学顺序直接返回，不再打乱！
    }
